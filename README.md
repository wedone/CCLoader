...
3. 勾选"开始时自动复位 CC2530"（默认开启）
   - 开启：进入监控时固件自动拉低 GPIO5/RESETn 10ms，CC2530 从 `main()` 重新执行
   - 关闭：只切换 Serial 到 CC2530 波特率，不影响 CC2530 当前状态
4. 点"开始监控"
5. 状态变为"监控中 @ 115200 bps"
6. **自动复位模式下**：立即看到从 `main()` 第一行开始的完整启动日志
7. **手动复位**：任何时候点"复位 CC2530"按钮，固件通过 GPIO5 拉低 RESETn 触发复位
   - 监控中复位会自动清空日志区，准备接收新的启动日志
   - 不需要手动按 CC2530 的 RESET 按钮，也不需要把 RESETn 接地
8. 工具栏功能：
   ...
   - **搜索过滤**：只显示包含关键字的行
9. 接收字节数实时显示在底部
10. 点"停止"退出监控，Serial 恢复 115200，可继续烧录

> **关于 RESETn 接线**：RESETn 已接 GPIO5 (D1)，固件通过 `digitalWrite` 控制，**不需要手动接地**。监控页的"复位 CC2530"按钮随时可用，烧录完成后也会自动复位。
...
- **HTTP REST API**（详见源码 `initHttpRoutes()`）：
  ...
  - `POST /api/stop` - 停止监控
  - `POST /api/reset` - 复位 CC2530（通过 GPIO5/RESETn，监控中也可用）
...
- **SSE 事件**（端口 81，`event: message`，`data` 为 JSON）：
  ...
  - `{"type":"monitor_stop"}`
  - `{"type":"monitor_reset"}` - CC2530 已复位（监控中），前端清空日志区准备接收启动日志
